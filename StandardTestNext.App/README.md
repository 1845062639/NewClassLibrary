...
- 已补 `RuntimeConfigurationValidator` + `RuntimeConfigurationConsoleReporter`，启动时会打印当前配置摘要，并对 provider、port、clientId、topicPrefix、samplingMode 做最小告警；本轮进一步修正为：`mqtt` 目前只在配置层被识别，尚未在 `MessageBusFactory` 实现，启动阶段会明确提示该差距，避免“校验通过但运行直接抛异常”的假绿状态
