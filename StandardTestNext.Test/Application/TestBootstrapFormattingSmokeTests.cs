...
public static class TestBootstrapFormattingSmokeTests
{
    ...
    private static void ShouldExposeAlgorithmFamilyPrimaryFieldFocusesInCliPlanPreview()
    {
        ...
        if (!formatted.Contains("anchor-family=GB:1:100.0 %:weighted=7:100.0 %:methods=51:method-keys=LoadB:51:profiles=delivery:legacy-methods=B法负载试验:settings-methods=B法负载试验:baseline-route=method=5, method-key=LoadB:5, profile=baseline:dominant-route=method=51, method-key=LoadB:51, profile=delivery:selected-route=method=51, method-key=LoadB:51, profile=delivery:upstream=:upstream-legacy=:upstream-hints=:MotorY.LoadB:families=LoadB:blocking:summary=algorithm-family decision-anchor primary fields top 1/1: GB=1 (100pp, weighted 7/7)", StringComparison.Ordinal)
            ...
            || !formatted.Contains("anchor-variant=GB:1:100.0 %:weighted=7:100.0 %:baseline=2:22.2 %:dominant=7:77.8 %:selected=7:77.8 %:methods=51:method-keys=LoadB:51:profiles=delivery:legacy-methods=B法负载试验:settings-methods=B法负载试验:baseline-route=method=5, method-key=LoadB:5, profile=baseline:dominant-route=method=51, method-key=LoadB:51, profile=delivery:selected-route=method=51, method-key=LoadB:51, profile=delivery:upstream=:upstream-legacy=:upstream-hints=:MotorY.LoadB:variants=delivery:blocking:summary=variant-kind decision-anchor primary fields top 1/1: GB=1 (100pp, weighted 7/7)", StringComparison.Ordinal)
