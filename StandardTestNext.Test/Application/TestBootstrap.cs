...
public sealed class TestBootstrap
{
    ...
    private static string FormatMethodAdaptationPlanSnapshot(MotorYMethodAdaptationPlanSnapshot plan)
    {
        ...
        var crossPlanAnchorPrimary = plan.CrossPlanDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-cross-plan=" + string.Join("|", plan.CrossPlanDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.CrossPlanDecisionAnchorPrimaryFieldSummary}";
        var algorithmFamilyAnchorPrimary = plan.AlgorithmFamilyDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-family=" + string.Join("|", plan.AlgorithmFamilyDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.AlgorithmFamilyDecisionAnchorPrimaryFieldSummary}";
        var variantKindAnchorPrimary = plan.VariantKindDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-variant=" + string.Join("|", plan.VariantKindDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:variants={string.Join("/", x.VariantKinds)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.VariantKindDecisionAnchorPrimaryFieldSummary}";
