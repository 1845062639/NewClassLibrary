...
public sealed class TestBootstrap
{
    ...
    private static string FormatCrossPlanPrimaryFieldFocuses(IReadOnlyList<MotorYPrimaryFieldFocusSnapshot> focuses)
    {
        ...
        return string.Join(", ", focuses.Select(focus =>
        {
            var weightedBase = totalWeighted > 0
                ? totalWeighted
                : focus.WeightedCount;
            return $"{focus.PrimaryField}:{focus.Count}:{focus.Share:P1}:weighted={focus.WeightedCount}/{weightedBase}:{focus.WeightedShare:P1}:methods={FormatPreview(focus.MethodValues.Select(x => x.ToString(CultureInfo.InvariantCulture)).ToArray(), 4)}:method-keys={FormatPreview(focus.MethodKeys, 4)}:profiles={FormatPreview(focus.ProfileKeys, 4)}:legacy-methods={FormatPreview(focus.LegacyMethodNames, 4)}:settings-methods={FormatPreview(focus.SettingsMethodNames, 4)}:algo-entries={FormatPreview(focus.LegacyAlgorithmEntries, 4)}:{FormatPreview(focus.CanonicalCodes, 3)}:{FormatPreview(focus.AnchorKeys, 3)}:{FormatPreview(focus.SuggestedNextStepPriorities, 3)}:summary={focus.Summary}";
        }));
    }
    ...
    private static string FormatMethodAdaptationPlanSnapshot(MotorYMethodAdaptationPlanSnapshot plan)
    {
        ...
        var algorithmFamilyAnchorPrimary = plan.AlgorithmFamilyDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-family=" + string.Join("|", plan.AlgorithmFamilyDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:method-keys={string.Join("/", x.MethodKeys)}:profiles={string.Join("/", x.ProfileKeys)}:legacy-methods={string.Join("/", x.LegacyMethodNames)}:settings-methods={string.Join("/", x.SettingsMethodNames)}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.AlgorithmFamilyDecisionAnchorPrimaryFieldSummary}";
        var variantKindAnchorPrimary = plan.VariantKindDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-variant=" + string.Join("|", plan.VariantKindDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:method-keys={string.Join("/", x.MethodKeys)}:profiles={string.Join("/", x.ProfileKeys)}:legacy-methods={string.Join("/", x.LegacyMethodNames)}:settings-methods={string.Join("/", x.SettingsMethodNames)}:{string.Join("/", x.CanonicalCodes)}:variants={string.Join("/", x.VariantKinds)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.VariantKindDecisionAnchorPrimaryFieldSummary}";
        ...
        var crossPlanResultPrimary = plan.CrossPlanRequiredResultPrimaryFieldFocuses.Count == 0
            ...
            : "result-cross-plan=" + string.Join("|", plan.CrossPlanRequiredResultPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.CrossPlanRequiredResultPrimaryFieldSummary}";
        var algorithmFamilyResultPrimary = plan.AlgorithmFamilyRequiredResultPrimaryFieldFocuses.Count == 0
            ...
            : "result-family=" + string.Join("|", plan.AlgorithmFamilyRequiredResultPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.AlgorithmFamilyRequiredResultPrimaryFieldSummary}";
        var variantKindResultPrimary = plan.VariantKindRequiredResultPrimaryFieldFocuses.Count == 0
            ...
            : "result-variant=" + string.Join("|", plan.VariantKindRequiredResultPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:methods={string.Join("/", x.MethodValues)}:profiles={string.Join("/", x.ProfileKeys)}:{string.Join("/", x.CanonicalCodes)}:variants={string.Join("/", x.VariantKinds)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.VariantKindRequiredResultPrimaryFieldSummary}";
