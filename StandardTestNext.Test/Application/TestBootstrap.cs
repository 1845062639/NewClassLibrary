...
public sealed class TestBootstrap
{
    ...
    private static string FormatMethodAdaptationPlanSnapshot(MotorYMethodAdaptationPlanSnapshot plan)
    {
        ...
        var crossPlanAnchorPrimary = plan.CrossPlanDecisionAnchorPrimaryFieldFocuses.Count == 0
            ...
            : "anchor-cross-plan=" + string.Join("|", plan.CrossPlanDecisionAnchorPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.CrossPlanDecisionAnchorPrimaryFieldSummary}";
        ...
        var crossPlanResultPrimary = plan.CrossPlanRequiredResultPrimaryFieldFocuses.Count == 0
            ...
            : "result-cross-plan=" + string.Join("|", plan.CrossPlanRequiredResultPrimaryFieldFocuses.Take(3).Select(x => $"{x.PrimaryField}:{x.Count}:{x.Share:P1}:weighted={x.WeightedCount}:{x.WeightedShare:P1}:{string.Join("/", x.CanonicalCodes)}:families={string.Join("/", x.AlgorithmFamilies)}:{string.Join("/", x.SuggestedNextStepPriorities)}")) + $":summary={plan.CrossPlanRequiredResultPrimaryFieldSummary}";
