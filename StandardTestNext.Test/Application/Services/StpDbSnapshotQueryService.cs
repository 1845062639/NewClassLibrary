...
public sealed class StpDbSnapshotQueryService
{
    ...
    public IReadOnlyList<MotorYPrimaryFieldFocusSnapshot> ListMotorYDecisionAnchorPrimaryFieldFocuses()
    {
        using var connection = new SqliteConnection($"Data Source={_dbPath}");
        connection.Open();
        return BuildCrossPlanDecisionAnchorPrimaryFieldFocuses(LoadMotorYMethodAdaptationPlans(connection));
    }
    ...
    private static IReadOnlyList<MotorYPrimaryFieldFocusSnapshot> BuildCrossPlanDecisionAnchorPrimaryFieldFocuses(IReadOnlyList<MotorYMethodAdaptationPlanSnapshot> plans)
    {
        if (plans.Count == 0)
        {
