    ...
    private static IReadOnlyList<MotorYMethodRecommendationSnapshot> LoadMotorYMethodRecommendations(SqliteConnection connection)
    {
        return LoadMotorYMethodDecisions(connection)
            .Select(decision => new MotorYMethodRecommendationSnapshot
            {
                ...
                BaselineVariantKind = decision.BaselineRoute?.VariantKind,
                BaselineAlgorithmFamily = decision.BaselineRoute?.AlgorithmFamily,
                BaselineLegacyEnumName = decision.BaselineRoute?.LegacyEnumName,
                BaselineLegacyFormName = decision.BaselineRoute?.LegacyFormName,
                BaselineLegacyAlgorithmEntry = decision.BaselineRoute?.LegacyAlgorithmEntry,
                BaselineLegacyMethodName = decision.BaselineRoute?.LegacyMethodName,
                BaselineLegacySettingsMethodName = decision.BaselineRoute?.LegacySettingsMethodName,
                BaselineSourceSections = GetSourceSections(decision.BaselineRoute),
                BaselineSourceRanges = GetSourceRanges(decision.BaselineRoute),
                BaselinePrimarySourceSection = GetPrimarySourceSection(decision.BaselineRoute),
                BaselinePrimarySourceRange = GetPrimarySourceRange(decision.BaselineRoute),
                BaselineFormNames = GetFormNames(decision.BaselineRoute),
                BaselineFormSourceRanges = GetFormSourceRanges(decision.BaselineRoute),
                BaselinePrimaryFormName = GetPrimaryFormName(decision.BaselineRoute),
                BaselinePrimaryFormSourceRange = GetPrimaryFormSourceRange(decision.BaselineRoute),
                BaselineIsBaselineMethod = decision.BaselineRoute?.IsBaselineMethod == true,
