...
public static class StpDbMotorYMethodRecommendationSmokeTests
{
    ...
    public static void Run()
    {
        ...
        var expected = new[]
        {
            BuildExpected(connection, MotorYTestMethodCodes.DcResistance, 1, 53),
            BuildExpected(connection, MotorYTestMethodCodes.NoLoad, 0, 0),
            BuildExpected(connection, MotorYTestMethodCodes.HeatRun, 3, 3),
            BuildExpected(connection, MotorYTestMethodCodes.LoadA, 4, 60),
            BuildExpected(connection, MotorYTestMethodCodes.LoadB, 5, 5),
            BuildExpected(connection, MotorYTestMethodCodes.LockedRotor, 11, 46)
        };
        ...
        int expectedDominantMethod)
    {
        ...
        if (!rows.TryGetValue(expectedDominantMethod, out var dominantCount))
        {
            throw new InvalidOperationException($"stp.db Motor_Y method recommendation smoke test failed: expected dominant method {expectedDominantMethod} missing for {canonicalCode}.");
        }
        ...
        var dominant = rows
            ...
            .ThenBy(x => x.Key)
            .First();
        if (dominant.Key != expectedDominantMethod)
        {
            throw new InvalidOperationException($"stp.db Motor_Y method recommendation smoke test failed: dominant source row changed for {canonicalCode}. expected={expectedDominantMethod}, actual={dominant.Key}");
        }

        return (canonicalCode, rows.Values.Sum(), baselineMethod, baselineCount, expectedDominantMethod, dominantCount);
    }
