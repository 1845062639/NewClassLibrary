...
public static class StpDbMotorYMethodRouteSelectionSmokeTests
{
    ...
    public static void Run()
    {
        ...
        {
            throw new InvalidOperationException($"stp.db not found for method route selection smoke test: {DbPath}");
        }

        var service = new StpDbSnapshotQueryService(DbPath);
        var actual = service.ListMotorYMethodRouteSelections();
        if (actual.Count == 0)
        {
            throw new InvalidOperationException("stp.db Motor_Y method route selection smoke test failed: no rows returned.");
        }
        ...
        foreach (var row in expected)
        {
            ...
            if (snapshot is null)
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: missing row for {row.CanonicalCode}.");
            }
            ...
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: numeric mismatch for {row.CanonicalCode}.");
            }
            ...
            if (Math.Abs(snapshot.BaselineShare - row.BaselineShare) > 0.0001d
                || Math.Abs(snapshot.DominantShare - row.DominantShare) > 0.0001d
                || Math.Abs(snapshot.SelectedShare - row.SelectedShare) > 0.0001d)
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: share mismatch for {row.CanonicalCode}.");
            }

            if (!string.Equals(snapshot.SelectionStrategy, row.SelectionStrategy, StringComparison.Ordinal)
                || snapshot.ShouldUseDominantRoute != row.ShouldUseDominantRoute)
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: strategy mismatch for {row.CanonicalCode}.");
            }

            if (Math.Abs(snapshot.DominantOverrideThreshold - DominantOverrideThreshold) > 0.0001d
                || snapshot.DominantLeadCount != row.DominantLeadCount
                || snapshot.DominantLeadPercentagePoints != row.DominantLeadPercentagePoints
                || snapshot.SelectedLeadCountVsBaseline != row.SelectedLeadCountVsBaseline
                || snapshot.SelectedLeadPercentagePointsVsBaseline != row.SelectedLeadPercentagePointsVsBaseline)
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: lead summary mismatch for {row.CanonicalCode}.");
            }

            if (!string.Equals(snapshot.SelectionReason, row.SelectionReason, StringComparison.Ordinal)
                || !string.Equals(snapshot.SelectedMethodSummary, row.SelectedMethodSummary, StringComparison.Ordinal)
                || !string.Equals(snapshot.BaselineDominantComparisonSummary, row.BaselineDominantComparisonSummary, StringComparison.Ordinal))
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: summary mismatch for {row.CanonicalCode}.");
            }

            if (snapshot.Distributions.Count != row.Distributions.Count)
            {
                throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: distribution count mismatch for {row.CanonicalCode}.");
            }
        }
    }
    ...
    private static void AssertRoute(MotorYLegacyAlgorithmRoute? route, string canonicalCode, int methodValue, string context)
    {
        var expected = MotorYLegacyAlgorithmRouteResolver.Resolve(canonicalCode, methodValue)
            ?? throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: expected route missing for {context}.");

        if (route is null)
        {
            throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: route missing for {context}.");
        }
        ...
        int baselineMethod)
    {
        ...
        if (rows.Count == 0)
        {
            throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: no source rows for {canonicalCode}.");
        }

        var baselineCount = rows.TryGetValue(baselineMethod, out var baseline) ? baseline : 0;
        var dominant = rows.OrderByDescending(x => x.Value).ThenBy(x => x.Key).First();
        var totalCount = rows.Values.Sum();
        var baselineShare = totalCount <= 0 ? 0d : Math.Round((double)baselineCount / totalCount, 4, MidpointRounding.AwayFromZero);
        var dominantShare = totalCount <= 0 ? 0d : Math.Round((double)dominant.Value / totalCount, 4, MidpointRounding.AwayFromZero);
        var shouldUseDominantRoute = baselineMethod != dominant.Key && dominantShare >= DominantOverrideThreshold;
        ...
        var selectedShare = totalCount <= 0 ? 0d : Math.Round((double)selectedCount / totalCount, 4, MidpointRounding.AwayFromZero);
        var selectionStrategy = shouldUseDominantRoute ? "dominant-threshold-over-baseline" : "baseline";
        ...
        var selectedLeadCountVsBaseline = Math.Max(0, selectedCount - baselineCount);
        ...
        var selectedRoute = MotorYLegacyAlgorithmRouteResolver.Resolve(canonicalCode, selectedMethod)
            ?? throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: selected route missing for {canonicalCode}:{selectedMethod}.");
        var baselineRoute = MotorYLegacyAlgorithmRouteResolver.Resolve(canonicalCode, baselineMethod)
            ?? throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: baseline route missing for {canonicalCode}:{baselineMethod}.");
        var dominantRoute = MotorYLegacyAlgorithmRouteResolver.Resolve(canonicalCode, dominant.Key)
            ?? throw new InvalidOperationException($"stp.db Motor_Y method route selection smoke test failed: dominant route missing for {canonicalCode}:{dominant.Key}.");
        ...
        var selectedMethodSummary = $"selected {selectedRoute.LegacyMethodName} method {selectedMethod} ({selectedRoute.VariantKind}) covering {selectedCount}/{totalCount} items ({selectedShare:P2})";
        ...
        var distributions = rows
            ...
            .ThenBy(x => x.Key)
            .Select(x => (x.Key, x.Value, totalCount <= 0 ? 0d : Math.Round((double)x.Value / totalCount, 4, MidpointRounding.AwayFromZero)))
            .ToArray();
