...
internal static class MotorYPrimaryFieldFocusFactory
{
    ...
    public static string BuildCrossPlanFocusSummary(string scope, IReadOnlyList<MotorYPrimaryFieldFocusSnapshot> focuses)
    {
        ...
        return $"cross-plan {scope} primary fields top {Math.Min(3, focuses.Count)}/{focuses.Count}: {string.Join("; ", preview)}; dominant={top.PrimaryField}@families={topFamilies}@codes={topCodes}@methods={topMethodValues}@method-keys={topMethodKeys}@legacy-methods={topLegacyMethods}@settings-methods={topSettingsMethods}@algo-entries={topLegacyAlgorithmEntries}@forms={topFormNames}@form-ranges={topFormRanges}@baseline={top.BaselineCount}/{top.Count}@baseline-weighted={top.BaselineWeightedCount}/{top.WeightedCount}@dominant-share={top.DominantCount}/{top.Count}@dominant-weighted={top.DominantWeightedCount}/{top.WeightedCount}@selected-share={top.SelectedCount}/{top.Count}@selected-weighted={top.SelectedWeightedCount}/{top.WeightedCount}@upstream={topUpstreamCodes}@upstream-hints={topUpstreamHints}";
    }
